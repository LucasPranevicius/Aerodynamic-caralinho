...
#                                        2025
#                           Distribuição Uniforme de Painéis
#======================================================================================
...
def gerar_malha_uniforme(corda, num_paineis):
    """
    Gera uma distribuição uniforme de painéis.
    """
    x = np.linspace(0, 1, num_paineis + 1)  # Distribuição linear uniforme
...
def gerar_tabela_completa(corda_frente, corda_tras, num_paineis_totais):
    ...
    # Distribuição uniforme para ambas as seções
    malha_frente = gerar_malha_uniforme(corda_frente, num_paineis_frente)
    malha_tras = gerar_malha_uniforme(corda_tras, num_paineis_tras) + corda_frente
...
salvar_em_excel(df_malha_completa, "cte.xlsx")
