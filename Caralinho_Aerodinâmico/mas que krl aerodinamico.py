...
def gerar_malha(corda, num_paineis):
    # Cria uma base linear de ângulos de 0 a 1
...
def gerar_tabela_completa(corda_frente, corda_tras, num_paineis_totais):
    ...
    malha_frente = gerar_malha(corda_frente, num_paineis_frente)
    malha_tras = gerar_malha(corda_tras, num_paineis_tras) + corda_frente

    corda_total = corda_frente + corda_tras
    prop_frente = malha_frente / corda_total
    prop_tras = malha_tras / corda_total

    prop_combinada = np.concatenate((prop_frente, prop_tras[1:]))

    # Função para padronizar os arrays para o mesmo comprimento, preenchendo com NaN
...
nome_do_arquivo = "malha_teste3.xlsx"
